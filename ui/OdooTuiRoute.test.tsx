...
if (!desktopGrid?.className.includes('lg:grid-cols-[minmax(16rem,0.25fr)_minmax(0,0.75fr)]') || !desktopGrid.className.includes('w-full')) throw new Error('desktop workspace must give the log column most of the width and fill its parent');
...
if (!desktopDatabaseContext?.className.includes('hidden') || !desktopDatabaseContext.className.includes('lg:block')) throw new Error('desktop database context must stay hidden on mobile');
if (!view.container.querySelector('[data-testid="client-selection"] [data-testid="database-summary"]')?.parentElement?.className.includes('lg:hidden')) throw new Error('mobile database summary must remain inside Instance');
if (!view.container.querySelector('[data-testid="desktop-workspace-grid"]')?.className.includes('lg:flex-1') || !view.container.querySelector('[data-testid="desktop-workspace-grid"]')?.className.includes('lg:min-h-0')) throw new Error('desktop workspace must consume only the remaining height');
if (!desktopLeft?.className.includes('lg:h-full') || !desktopLeft.className.includes('lg:flex-col') || !desktopLeft.className.includes('lg:min-h-0')) throw new Error('desktop left area must fill the grid row without owning page scroll');
...
if (!modulePanel?.className.includes('lg:flex-1') || !modulePanel.className.includes('lg:flex-col') || !modulePanel.className.includes('lg:min-h-0')) throw new Error('desktop module area must consume the remaining left-column height');
if (!view.container.querySelector('[data-testid="module-list"]')?.className.includes('lg:flex-1') || !view.container.querySelector('[data-testid="module-list"]')?.className.includes('lg:overflow-y-auto')) throw new Error('desktop module list must own the internal scroll');
if (!desktopGrid.querySelector('[data-testid="odoo-logs"]')?.className.includes('w-full') || !desktopGrid.querySelector('[data-testid="odoo-logs"]')?.className.includes('lg:h-full') || !desktopGrid.querySelector('[data-testid="odoo-logs"]')?.className.includes('lg:min-h-0')) throw new Error('desktop log panel must fill the remaining column width and height');
if (!view.container.querySelector('[data-testid="odoo-log-viewport"]')?.className.includes('lg:max-h-none') || !view.container.querySelector('[data-testid="odoo-log-viewport"]')?.className.includes('lg:flex-1')) throw new Error('desktop log viewport must consume the remaining panel height');
