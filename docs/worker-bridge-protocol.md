...
The plugin telemetry process cannot import `odoo_tui` because it runs in the Mission Control telemetry virtualenv. `worker_bridge.OdooTuiWorkerBridge` starts the explicitly configured odoo-tui virtualenv interpreter and sends one JSON request on stdin. The worker returns exactly one JSON response on stdout and never opens a port. The bridge implements the adapter transport seam for the six read operations: `clients.list`, `releases.list`, `instance.identity`, `runtime.status`, `modules.list`, and `databases.list`. The strict worker envelope is internal; public plugin HTTP handlers return the typed `data` object, while host dispatch serializes failures as `{\"error\": code, \"detail\": message}`.

## Deployment configuration

`WorkerConfig.interpreter`, `WorkerConfig.worker`, and `WorkerConfig.config_path` are deployment configuration. They must be absolute paths and are never accepted from `/api/local` input. The default interpreter is the existing odoo-tui virtualenv:
...
Only the six read operations are allowlisted. Client and environment identifiers are strict slugs. There is deliberately no `start`, `stop`, `restart`, `cycle`, `update`, or onboarding operation.
...
The bridge maps transport failures to stable uppercase `BridgeError.code` values: `OPERATION_NOT_ALLOWED`, `INVALID_REQUEST`, `DEPENDENCY_UNAVAILABLE`, `TIMEOUT`, `PROCESS_CRASH`, and `MALFORMED_RESPONSE`. Worker errors use stable uppercase codes and their messages are replaced with fixed safe text rather than forwarding worker exception text. Error messages never include request secrets or captured stderr.
